...
function get_point_time_hint(ques_time,ques_point){
    ...
    var str='<table><caption><span class="color-success">Trả lời đúng</span> trong khoảng thời gian quy định bạn sẽ được  + <span class="color-danger">số điểm</span> như sau:</caption><tbody>';
          ...
          str+= '<tr><td><span class="color-quote">Trên </span> ' + (hs*4) +' phút</td>';
...
var bs_lisTPL = {
    ...
    result: base_urlroot + "/file/luyenthi123/common/lesson/pho_thong/temps/result.htm",
...
var link_css = base_urlroot + "/file/luyenthi123/common/lesson/pho_thong/css/style.css?v=28092017";
var video_w = 640,
    video_h = 360;
...
if (typeof is_tablet != UNF) {
    link_css = base_urlroot + "/file/luyenthi123/common/lesson/pho_thong/css/t_style.css?vs=28092017";
...
function start_lesson() {
    ...
    if(data_lesson.lesson.save == 0){
        ...
        if (data_lesson.segment.length <= 3) {
           $('#basic_main').append(msg_non_vip_sb);
        	...
        	for(var i=1;i<= data_lesson.segment.length;i++){
    ...
    // if($('.bs_part_content').length>0){  
    //  if(!paidmember()){
    //      $('.bs_part_content').html(msg_non_vip_sb);
    //  }           
    //  $('.bs_part_content').after('<span class="open_more" show="0"  title="click để xem thêm nội dung">[xem thêm]</span>');
    //  $('.open_more').click(function(){
    //      $(this).prev().slideToggle();   
    //      var _show=$(this).attr('show');
    //      if(_show==0){
    //          $(this).attr('show',1);
    //          $(this).text('[rút gọn]');
    ...
    //      $(this).after('<div class="space5"></div>');
    //  });
    // }
    ...
    var url_action = base_urlroot + '/file/luyenthi123/common/lesson/pho_thong/js/action.js?vs=28092017';
...
function loadHtml() {
    ...
    // $.getScript("https://test.luyenthi123.com/libraries/MathJax/MathJax.js?config=TeX-AMS_HTML-full",function(){                 
    //         MathJax.Hub.Register.StartupHook("End",function (e) {
    //          console.log(e);
    //             $('#basic_box').css({visibility:'visible'});
    //             $('.ic_loading').css({visibility:'hidden'});
    ...
    //   js.src = "//connect.facebook.net/en_GB/sdk.js#xfbml=1&version=v2.6&appId=687979838022541";
    //   fjs.parentNode.insertBefore(js, fjs);
    // }(document, 'script', 'facebook-jssdk'));        
    // $('#basic_box').after('<div class="control_sharefb" style="display:none"><div class="control_sharefb_left">Xin chúc mừng bạn đã làm đúng <span class="fbuser_score"></span>.<br><span class="txt_alert_fb">Chia sẻ kết quả này với bạn bè trên Facebook</span></div><div class="control_sharefb_right"><div class="fb-share-button" data-href="'+link_share+'" data-layout="button_count" data-mobile-iframe="true"></div></div></div>');
